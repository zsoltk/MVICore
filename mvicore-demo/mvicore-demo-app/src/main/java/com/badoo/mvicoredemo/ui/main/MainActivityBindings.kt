...
import com.badoo.mvicore.PairBuffer
...
//        binder.bind(combineLatest(feature1, feature2) to view using ViewModelTransformer() + PairBuffer<ViewModel>() named "MainActivity.ViewModels")
...
operator fun <A, B, C> ((A) -> B).plus(f: (B) -> C): (A) -> C = { f(this(it)) }
